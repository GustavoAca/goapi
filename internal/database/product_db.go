...
import (
	...
	"github.com/GustavoAca/goapi/internal/entity"
)
...
func (pb *ProductDB) GetProducts() ([]*entity.Product, error) {
	rows, err := pb.db.Query("SELECT id, name, description, price, category_id, image_url FROM products")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
